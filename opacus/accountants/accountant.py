...
from opacus.accountants.analysis import mechanism
...
class SAccountant(abc.ABC):
    @abc.abstractmethod
    def __init__(self,mechanism : mechanism):
        self.mechanism = mechanism
        self.history = []  # history of random mechanism, and subsampling param, number of steps

    @abc.abstractmethod
    def step(self, *, m: mechanism, sample_rate: float):
        """
        Signal one optimization step

        Args:
            mechanism: The randomized mechanism to used
            sample_rate: Current sample rate
        """
        pass
    ...
    def state_dict(self, destination: T_state_dict = None) -> T_state_dict:
        ...
        if destination is None:
            destination = OrderedDict()
        destination["history"] = deepcopy(self.history)
        destination["mechanism"] = deepcopy(self.mechanism)
        return destination
