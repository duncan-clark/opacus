...
class RDPGeneralSubsampledAccountant(SAccountant):
    DEFAULT_ALPHAS = [1 + x / 10.0 for x in range(1, 100)] + list(range(12, 64))

    def __init__(self,mechanism):
        super().__init__(mechanism = mechanism)

    def step(self, *, sample_rate: float,m: m = None):
        if m is None:
            m = self.mechanism
        if len(self.history) >= 1:
            last_m, last_sample_rate, num_steps = self.history.pop()
            if (
                last_m == m
                and last_sample_rate == sample_rate
            ):
                self.history.append(
                    (last_m, last_sample_rate, num_steps + 1)
                )
            else:
                self.history.append(
                    (last_m, last_sample_rate, num_steps)
                )
                self.history.append((m, sample_rate, 1))
    ...
    def get_privacy_spent(
        ...
        rdp = sum(
            [
                privacy_analysis.compute_general_subsampled_rdp_bound(
                    m = m,
                    q=sample_rate,
                    steps=num_steps,
                    orders=alphas,
                )
                for (m, sample_rate, num_steps) in self.history
            ]
        )
        eps, best_alpha = privacy_analysis.get_privacy_spent(
            orders=alphas, rdp=rdp, delta=delta
